 ...
 * @author dell
 */
public class DAOSalesRep {
  //set Table
    private static final String TABAL = "SalesRep";
    
    static void save(OBJSalesRep obj, Connection conn, int Act) throws SQLException {
        if (Act == 1) {
            String sql = "INSERT INTO "+TABAL+" ( RepCode, RepName, AreaCode, Address1, Address2, Address3, TelNo, FaxNo, TargetAmt, POBox, Email, id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            String sql = "UPDATE "+TABAL+" SET  Email = ?, RepName = ?, AreaCode = ?, Address1 = ?, Address2 = ?, Address3 = ?, TelNo = ?, FaxNo = ?, TargetAmt = ?, POBox = ?, id = ? WHERE RepCode = '" + obj.getCode() + "'";
            PreparedStatement st = conn.prepareStatement(sql);            
    ...
    static OBJSalesRep getNavi(Connection conn, int Ix) throws SQLException {
        ...
        //MySql  
        //Ix =-1;
        String sql = "SELECT * FROM "+TABAL+" WHERE Status = '0' ORDER BY RepCode ASC LIMIT "+Ix+",1";
    ...
    static OBJSalesRep serch(Connection conn, String code) throws SQLException {

        OBJSalesRep obj = null;
        String sql = "SELECT * FROM "+TABAL+" WHERE RepCode = '" + code + "'";
        PreparedStatement st = conn.prepareStatement(sql);
        st.execute();
        ResultSet result = st.getResultSet();
        while (result.next()) {
    ...
    static void delete(Connection conn, String code) throws SQLException {
        PreparedStatement st = conn.prepareStatement("DELETE FROM "+TABAL+" WHERE RepCode = '" + code + "'");
        st.execute();
        st.close();
    }
    ...
    static String genID(Connection conn) throws SQLException {
        String ID = "01";
        String sql = "SELECT RepCode FROM "+TABAL+" ORDER BY RepCode";
