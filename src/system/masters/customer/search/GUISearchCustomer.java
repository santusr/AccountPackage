...
import system.masters.customer.CustomerCellRender;
import system.masters.customer.CustomerStatus;
import system.masters.customer.GUIShowInvoices;
 ...
 * @author dell
 */
public class GUISearchCustomer extends javax.swing.JInternalFrame {
    ...
    private void setContentHistory() {
        ...
        obja = SERSearchCustomer.getContent("C");
        setContent();
        jTable1.setDefaultRenderer(Object.class, new CustomerCellRender());
        //  jTable1.setEnabled(!true);
    }

    private void doLSearch(String s) {
        obja = SERSearchCustomer.getContent("C", s);
        setContent();
        //  jTable1.setEnabled(!true);
    }
    ...
    private void doBlackList(String action) {
        ...
        if (row >= 0) {
            ...
            boolean b = SERSearchCustomer.setBlackList(customer, status);
            if (b) {
                JOptionPane.showMessageDialog(null, "Customer " + statust + " Succssful...");
                setContentHistory();
            }
        }
...
private void cmdNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdNewActionPerformed
    ...
    private void cmdCustomerHistoryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdCustomerHistoryActionPerformed
        ...
        if (i >= 0) {
            try {
                new PrintCustomerHistory(cc).setVisible(true);
            } catch (Exception ex) {
                Exp.Handle(ex);
            }
        }
    ...
    private ArrayList<OBJCustomer> obja;
    //status
