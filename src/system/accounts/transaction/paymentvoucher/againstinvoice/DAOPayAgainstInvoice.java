 ...
 * @author dell
 */
public class DAOPayAgainstInvoice {
     //set Table
    private static final String TABAL = "customerGroup";
    
    static void save(OBJPayAgainstInvoice obj, Connection conn, int Act) throws SQLException {
        if (Act == 1) {
...
//            st.setString(11, obj.getId());
            ...
            String sql = "UPDATE "+TABAL+" SET CustGrpname = ?, address1 = ?, address2 = ?, address3 = ?, pobox = ?, teloff = ?, faxno = ?, MobileNo = ?, Email = ?, Id = ? WHERE CustGrpCode = '" + obj.getVouNo()+ "'";
            PreparedStatement st = conn.prepareStatement(sql);
...
//            st.executeUpdate();
    ...
    static OBJPayAgainstInvoice getNavi(Connection conn, int Ix) throws SQLException {
...
//                    result.getString(11));
    ...
    static OBJPayAgainstInvoice serch(Connection conn, String code) throws SQLException {
