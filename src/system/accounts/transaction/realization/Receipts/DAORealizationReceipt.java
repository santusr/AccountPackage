...
final static String TABALHISTORY = "cheque";
    static ArrayList<OBJCheque> getContent(Connection conn, String r) throws SQLException {
        ...
        OBJCheque obj = null;
        String sql = "SELECT * FROM " + TABALHISTORY + " WHERE Status != '"+ChequeStatus.CANCEL+"' AND "+r+"";
        PreparedStatement st = conn.prepareStatement(sql);
        st.execute();
        ResultSet result = st.getResultSet();
        while (result.next()) {
            System.out.println(result.getString(4)+"--------Account -------"+result.getString(18));
            obj = new OBJCheque(
                    result.getString(1),
                    result.getString(2),
                    result.getString(3),
