...
import system.accounts.transaction.receiptvoucher.onaccount.OBJVoucherOnAccount;
...
public class DAOVouch {
    ...
    static String type = "RECEIPT";
    private static final String TABAL = "voucher";

    static void doSave(OBJAgInvo obj, Connection conn, int Act) throws SQLException {
        if (Act == 1) {
            String sql = "INSERT INTO " + TABAL + " (VoucherNo, InvoNo, Cust, VouDate, CostCenter, Narration, PayAmount, PayTerm, RefNo, PrepBy, ApproBy, user, type) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            st.setString(9, obj.getRefNo());
            st.setString(10, obj.getPrepBy());
            st.setString(11, obj.getAppBy());
            st.setString(12, accountpackage.AccountPackage.user);
            st.setString(13, obj.getType());

            st.execute();

        } else {
            String sql = "UPDATE " + TABAL + " SET InvoNo = ?, Cust = ?, VouDate = ?, CostCenter = ?, Narration = ?, PayAmount = ?, PayTerm = ?, RefNo = ?, PrepBy = ?, ApproBy = ?, user = ? WHERE VoucherNo = '" + obj.getVouNo() + "'";
            PreparedStatement st = conn.prepareStatement(sql);
...
//    
    ...
    static OBJAgInvo getVouch(Connection conn, int Ix) throws SQLException {
        ...
        //MySql
        //Ix =-1;
        String sql = "SELECT * FROM " + TABAL + " WHERE type = '" + type + "' ORDER BY VoucherNo LIMIT " + Ix + ",1";
        PreparedStatement st = conn.prepareStatement(sql);
        st.execute();
...
//                    result.getString(9));
    ...
    static OBJAgInvo serch(Connection conn, String code) throws SQLException {
        ...
        OBJAgInvo obj = null;
        String sql = "SELECT * FROM " + TABAL + " WHERE CreditId = '" + code + "' AND Status = '0' AND type = '" + type + "'";
        PreparedStatement st = conn.prepareStatement(sql);
        st.execute();
        ResultSet result = st.getResultSet();
        while (result.next()) {
...
//    }
    ...
    static int getIndex(Connection conn) throws SQLException {
        ...
        int index = 0;
        String sql = "SELECT COUNT(*) AS val FROM " + TABAL + " WHERE type = '" + type + "'";
    ...
    static void deleteVouch(Connection conn, String code) throws SQLException {
        ...
        String sql = "UPDATE " + TABAL + " SET Status = ? WHERE VoucherNo = '" + code + "' AND type = '" + type + "'";
    ...
    static String genID(Connection conn) throws SQLException {
        String ID = "REC000001";
        String sql = "SELECT VoucherNo FROM " + TABAL + " WHERE type = '" + type + "' ORDER BY VoucherNo DESC LIMIT 1";
        PreparedStatement st = conn.prepareStatement(sql);
        st.execute();
        ResultSet result = st.getResultSet();
        while (result.next()) {
